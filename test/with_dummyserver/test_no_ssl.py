...
Note: Import urllib3_gevent inside the test functions to get the importblocker to work
"""
...
class TestHTTPWithoutSSL(HTTPDummyServerTestCase, TestWithoutSSL):
    def test_simple(self):
        pool = urllib3_gevent.HTTPConnectionPool(self.host, self.port)
...
class TestHTTPSWithoutSSL(HTTPSDummyServerTestCase, TestWithoutSSL):
    def test_simple(self):
        pool = urllib3_gevent.HTTPSConnectionPool(self.host, self.port)
        self.addCleanup(pool.close)
        try:
            pool.request('GET', '/')
        except urllib3_gevent.exceptions.SSLError as e:
